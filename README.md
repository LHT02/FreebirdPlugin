...
- 在 `CUSTOM` 菜单切换到 `ADD POINT` 后，按住主手扳机拖过相邻控制节点之间的直线，可在命中位置连续插入控制点。
- 新增 spline 的节点半径可通过 `RADIUS -`、`RADIUS +` 设置。
- 支持 NURBS、POLY，并适配 Bézier 控制点的选择、移动、扭转和半径。
- `CUSTOM` 页面中的绘制、编辑、加点、衰减和半径按钮均带有插件自带图标。
...
4. 使用 `ADD POINT` 进入加点模式；按住主手扳机并拖过两个相邻控制节点之间的直线。拖动超过触发距离后，工具会在命中位置插点；继续沿线移动可连续添加，松开扳机结束本次操作。
5. 编辑节点时直接抓住并拖动：
   - 移动控制器改变位置；
   - 转动手腕改变曲线扭转；
   - 推动主手摇杆上下改变节点半径。
6. 使用 `FALLOFF ON/OFF` 切换衰减编辑。抓取前，Freebird 原有的摇杆比例范围调节仍然有效；开始抓取后，主手摇杆优先控制节点半径。

加点模式会忽略隐藏节点、退化线段和靠近已有节点的端点区域，并依据 VR 游标大小设置命中范围和最小插点间距。NURBS、POLY、Bézier 及循环 spline 均按各自相邻控制节点处理；点击 `EDIT` 可退出加点模式并恢复节点选择/拖动。
