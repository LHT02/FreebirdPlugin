# PROJECT

## 项目目标

开发 Freebird XR 的曲线编辑插件，为 Blender/VR 工作流提供直观的曲线创建与编辑能力。

## 开发原则

- 使用 Python 和 Freebird/Blender 提供的插件接口。
- 按职责拆分注册、UI、曲线操作和状态管理，避免上帝文件。
- Windows 自动化命令使用 PowerShell 7。
- 文本统一保存为 UTF-8（无 BOM）。
- 每次修改记录实现内容、验证结果和未完成事项。

## 固定开发基准

- Blender：`3.6.23 LTS`
- Blender 路径：`C:\blender\software\stable\blender-3.6.23-lts.e467db79ca8c\blender.exe`
- Freebird XR：本机当前安装版本 `2.14.2`，提交 `dd3cb84`
- Freebird 路径：`C:\Users\LHT02\AppData\Roaming\Blender Foundation\Blender\3.6\scripts\addons\freebird_xr`
- Freebird 官方变更日志当前处于 `2.14` 系列；开发时以本机实际安装的 `version.json` 为精确版本依据。

## 变更记录

### 2026-09-03：Git 仓库准备

完成内容：

- 确认工作目录初始为空且尚未初始化 Git。
- 查阅 Freebird 官方插件文档，确认插件是由 Freebird/Blender 加载的 Python 文件或包。
- 准备 `main` 分支的 Git 仓库基线。
- 新增 Python、Blender、Freebird 本地数据和常见编辑器文件的忽略规则。
- 新增换行符、UTF-8（无 BOM）及基本编辑格式约定。
- 新增项目说明与持续维护约定。
- 固定 Blender `3.6.23 LTS` 与本机 Freebird XR `2.14.2`（`dd3cb84`）为首个开发基准。

核验结果：

- 指定的 `blender.exe` 路径存在。
- 检测到该 Blender 当前已有运行实例，因此未启动新的后台实例，避免影响未保存工作。
- 用户级 Blender `3.6` 插件目录中存在 `freebird_xr`。
- `freebird_xr/version.json` 与入口 `bl_info` 均标记版本 `2.14.2`。
- 仓库文本文件已检查为 UTF-8 无 BOM，Git 属性固定普通文本为 LF、Windows 脚本为 CRLF。
- 首次提交前已通过 `git diff --cached --check` 检查。

当前状态：

- 尚未添加曲线编辑功能代码。
- 尚未配置远程仓库。
- 尚未在 Blender/Freebird 中执行插件加载测试。

下一步：

- 确认曲线编辑器的首版功能范围和交互流程。
- 建立 Freebird 插件包骨架及最小加载验证。
- 增加适用于纯 Python 逻辑的自动化测试，并记录 Blender/VR 实机验证结果。

### 2026-09-03：曲线编辑增强插件首版

实现内容：

- 新增 `freebird_curve_editor` Freebird 插件包，并通过版本保护限定 Freebird `2.14.2 / dd3cb84`。
- 将曲线碰撞检测、选择、选择状态、移动目标、擦除和事件目标校验扩展到一个 Curve 数据块中的全部 spline。
- 抓住曲线节点时，把实体控制器绕节点曲线切线的旋转分量映射到 `tilt`；没有新增旋转环，现有变换手柄拖动也不会触发该功能。
- 抓住曲线节点时，使用主手纵向摇杆按指数倍率缩放 `radius`，并在该上下文阻止 Walk 导航和比例范围处理争抢输入。
- 增加衰减编辑开关；依据 Blender 比例编辑半径、连接模式和衰减类型，对位置、扭转和半径统一施加权重。
- 将 Freebird 原有比例编辑范围球扩展到曲线编辑模式，便于在 VR 中观察衰减范围。
- 自动关键帧开启时，为直接编辑的曲线节点补充 `tilt`、`radius` 和 Bézier 手柄关键帧。
- 增加 `DRAW IN`/`EDIT` 模式切换，复用 Freebird NURBS 笔画采样与收尾逻辑，将新 spline 写入当前 Curve 对象。
- 增加新 spline 半径倍率的减小、显示和增大按钮；支持固定粗细或与控制器压力相乘。
- 增加 NURBS、POLY 和 Bézier 控制点适配、可逆补丁注册与卸载恢复。
- 增加安全的开发目录联接安装脚本和 Freebird 源码契约验证脚本。

自动验证：

- Blender `3.6.23` 自带 Python `3.10.13` 已完成插件与测试代码语法编译。
- 扭转角、摇杆倍率、衰减函数、多 spline 遍历、连接衰减、切线计算和可逆补丁共 13 项单元测试已通过。
- Freebird 源码契约检查已通过，所需模块、类和函数均与 `2.14.2 / dd3cb84` 匹配。
- 已创建并核验开发目录联接：`C:\Users\LHT02\.freebird\plugins\freebird_curve_editor` → `D:\FreebirdPlugin\freebird_curve_editor`。

待验证：

- 当前有 Blender 实例正在运行，为保护未保存工作，没有启动竞争的后台 Blender，也没有执行 `Reload All`。
- 仍需在安全时机完成 Freebird 插件真实加载检查。
- 仍需使用 VR 控制器确认扭转方向、摇杆手感、衰减范围和同对象连续绘制的实际交互。

### 2026-09-03：v0.1.0 GitHub 首版快照

发布内容：

- 将当前曲线编辑增强插件保存为 `v0.1.0` 首版快照。
- GitHub 仓库设为公开：`https://github.com/LHT02/FreebirdPlugin`。
- 推送 `main` 分支及 `v0.1.0` 标签。

发布前验证：

- 重新执行 Blender Python 语法编译、13 项单元测试与 Freebird 源码契约检查。
- 核对提交内容不存在 Git 空白错误。
- 不启动、不关闭或重载当前正在运行的 Blender 实例。

### 2026-09-03：v0.1.1 扭转稳定与工具图标

问题修复：

- 将逐帧旋转四元数规范到同一半球，使数学上等价的 `q` 与 `-q` 始终产生相同的最短弧扭转角，避免偶发接近整圈的错误增量。
- 在每次抓取开始时冻结各受影响曲线节点的世界空间切线，防止垂直曲线、内收 NURBS 或节点跨越相邻点时切线方向翻转并累积成“扭麻花”。
- 拒绝单帧超过 45 度的控制器旋转差值，将其视为 VR 追踪中断或恢复产生的跳变；正常逐帧手腕旋转保持不变。
- 对无效、非有限或零长度四元数采用中性结果，不再把异常输入写入节点 `tilt`。

界面完善：

- 为 `DRAW IN`、`EDIT`、`FALLOFF ON/OFF`、`RADIUS -`、半径显示和 `RADIUS +` 提供 7 个独立的 128×128 RGBA PNG 图标。
- 图标作为插件资源随仓库分发，并以绝对路径传给 Freebird `add_launcher_button(..., icon=...)`，不依赖外部资源。
- 保留可重复生成图标的 `tools/generate_icons.py`，避免二进制资源无法追溯。

验证结果：

- Blender `3.6.23` 自带 Python 已通过全部插件、测试及图标生成脚本的语法编译。
- 自动测试由 13 项增至 20 项，新增四元数双覆盖等价性、追踪跳变拒绝、无效输入、抓取切线冻结和图标格式/路径检查，全部通过。
- Freebird `2.14.2 / dd3cb84` 源码契约检查通过。
- 7 个图标均已人工查看；尺寸、RGBA 模式和透明背景符合 Freebird 深色菜单使用需求。

待实机验证：

- 当前未重载正在运行的 Blender，仍需在可安全重载时用 VR 控制器复测垂直曲线、内收 NURBS、快速手腕转动及所有按钮图标显示。

### 2026-09-03：v0.2.0 线段拖动加点工具

实现内容：

- 新增独立 Freebird 工具 `curve.add_point` 和 `ADD POINT` 启动按钮；启用时 Freebird 会关闭 `select` 工具及变换手柄，避免在线段上操作时误选或误拖原节点。
- 按住主手扳机并拖过相邻控制节点之间的直线时，以控制器位置到控制线段的最近点作为插入位置。
- 支持一个 Curve 数据块中的全部 NURBS、POLY、Bézier spline，并支持三个及以上控制点的循环 spline 收尾线段。
- 复用 Blender `bpy.ops.curve.subdivide(number_cuts=1)` 完成拓扑插入，保留 Blender 对 Bézier 手柄、NURBS 节点、形态键索引和动画路径的原生维护；随后把新增点移动到实际命中位置，并按线段比例插值 `radius`、`tilt` 和软体权重。
- 加入拖动起步阈值、按 VR 游标缩放的线段命中半径、端点保护及最小连续插点间距，避免单击误加、已有节点附近重复加点和控制器停留时爆量加点。
- 多对象编辑时临时屏蔽其他 Curve 对象的节点选择，防止 Blender 的多对象细分操作误改非目标对象，操作结束立即恢复其选择状态。
- 新增第 8 个插件内置图标 `add_point.png`，以线段、两端节点和中央加号表达加点操作。
- 插件版本更新为 `0.2.0`。

验证结果：

- Blender `3.6.23` 自带 Python 已通过插件、测试和图标生成脚本语法编译。
- 自动测试由 20 项增至 27 项，新增自定义工具注册/恢复、开放/循环线段枚举、多 spline 最近线段、隐藏与端点排除、开放/循环插入索引，以及新增点位置、属性插值、选择隔离检查，全部通过。
- Freebird `2.14.2 / dd3cb84` 源码契约检查新增 `_get_modules`、`enable_tool` 与 `disable_tool` 检查并通过。
- 加点图标已人工检查为透明背景 128×128 RGBA PNG；开发目录联接会直接读取新增代码与资源。
- 实现依据 Blender 3.6 官方 `curve.subdivide` 操作接口及 Blender `v3.6.23` 对 legacy Curve 细分的源码插入顺序。

待实机验证：

- 当前仍有多个 Blender 实例运行，为保护未保存工作未执行 `Reload All` 或竞争性后台 Blender 测试。
- 仍需在 VR 中确认游标命中宽容度、连续插点间距、循环段和复杂 Bézier/NURBS 的实际手感；自动验证不替代该项。
